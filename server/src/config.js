...
export function createConfig(env = process.env) {
  ...
  const gasSeedEnabled = booleanValue(env.VENDOR_GAS_SEED_ENABLED, false);
  const seedAmountUsdc = positiveNumber(env.VENDOR_GAS_SEED_USDC, 0.1);
  const maxSeedAmountUsdc = positiveNumber(env.MAX_VENDOR_GAS_SEED_USDC, 0.2);
  ...
  if (seedAmountUsdc > maxSeedAmountUsdc) configurationErrors.push("VENDOR_GAS_SEED_USDC exceeds MAX_VENDOR_GAS_SEED_USDC.");
  ...
  return Object.freeze({
    ...
    metadataFile: env.VENDOR_WALLET_METADATA_FILE?.trim() || new URL("../data/vendor-wallets.json", import.meta.url),
  });
}
